...
"""
Myfxbook ROI Reporter -> Telegram
- Đăng nhập Myfxbook (email/password từ ENV)
- Chọn account (MYFXBOOK_ACCOUNT_ID nếu có, không thì lấy account đầu tiên)
- Tính ROI Day / Week / Month bằng get-daily-gain (lãi kép)
- Lấy All từ metadata account (gain tổng)
...
# ============== Utils ==============
def now_vn() -> datetime:
    """Trả về datetime theo múi giờ từ ENV (mặc định Asia/Ho_Chi_Minh)."""
    tzname = os.getenv("TZ", "Asia/Ho_Chi_Minh")
    try:
        # chỉ xử lý sẵn Asia/Ho_Chi_Minh; còn lại trả giờ local
...
def main() -> None:
    ...
    try:
        ...
        logging.info(
            "Using account id=%s, name=%s, broker=%s",
        ...
        for key, (start, end) in rg.items():
            try:
                ...
                logging.info(
                    "ROI %s %s→%s | days=%s | simple=%s | compound=%s",
